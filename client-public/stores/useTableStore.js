...
export const useTableStore = create((set, get) => ({
	...
	init: async (tableId = null, restaurantId = null) => {
		try {
			...
			const finalTableId = tableId || savedTableId || API_CONFIG.DEFAULT_TABLE_ID;
			const finalRestaurantId = restaurantId || savedRestaurantId || API_CONFIG.RESTAURANT_ID;
	...
	joinTable: async (userName, tableId = null, restaurantId = null) => {
		try {
			...
			const finalRestaurantId = restaurantId || state.restaurantId || API_CONFIG.RESTAURANT_ID;

			if (!finalTableId) {
				throw new Error("TableId manquant. Veuillez scanner le QR code de la table.");
			}
