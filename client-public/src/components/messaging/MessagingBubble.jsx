/**
 * 💬 MessagingBubble - Chat moderne client ↔ serveur
 *
 * Design inspiré de chat-bubble (21st.dev)
 * - Bulle flottante avec animation
 * - Conversation style "chat bubbles"
 * - Suggestions de messages prédéfinis (pas d'input)
 */
...
const COLORS = {
	...
	primaryLight: "#818cf8",
	primaryDark: "#4f46e5",
	sent: "#6366f1", // Messages envoyés (client)
	received: "#f1f5f9", // Messages reçus (serveur)
	receivedText: "#1e293b",
	background: "#ffffff",
	surface: "#f8fafc",
	border: "#e2e8f0",
...
const CATEGORY_CONFIG = {
	service: { emoji: "🍽️", label: "Service" },
	commande: { emoji: "📝", label: "Commande" },
	paiement: { emoji: "💳", label: "Paiement" },
	autre: { emoji: "💬", label: "Autre" },
};

// 🎯 Messages prédéfinis par défaut (fallback si API échoue)
...
const MessagingBubble = ({ reservationId, clientId, clientName, style }) => {
	...
	// Handlers stables (ne changent JAMAIS pour éviter re-trigger du useEffect WebSocket)
	const handleServerResponse = useCallback((event) => {
		if (event.data?.reservationId !== reservationIdRef.current) return;
		...
		// Utiliser le ref au lieu de isOpen direct
	...
	// Animation apparition bulle + charger messages prédéfinis
	useEffect(() => {
		const timer = setTimeout(() => {
			...
			startPulseAnimation();
			// 🎯 Charger les messages prédéfinis dès l'apparition de la bulle
			loadMessages();
		}, 2500);
	...
	const sendMessage = useCallback(
		async (message) => {
			...
			try {
				// 🎯 Pour les messages par défaut, on les ajoute juste localement
				// car le backend n'a pas ces IDs en DB
				const isDefaultMessage = message._id.toString().startsWith("default-");

				if (isDefaultMessage) {
					// Ajouter directement à la conversation sans appeler l'API
					setConversation((prev) => [
						...prev,
						{
							type: "client",
					...
					setConversation((prev) => [
						...prev,
						{
							...
							text: message.text,
							createdAt: new Date().toISOString(),
							_id: realMessageId || Date.now().toString(),
							status: "sent",
						},
					]);
				}
	...
	// Grouper les catégories disponibles
							...
							{conversation.length === 0 && (
								<View style={styles.welcomeContainer}>
									...
									<Text style={styles.welcomeText}>
										Sélectionnez un message ci-dessous pour appeler le serveur
									</Text>
								</View>
							)}
